...
---- - 経理部に所属している社員一覧
--SELECT DISTINCT id, name, birthday, sex, joining_date, position_id, department_id
--FROM worker
...
--SELECT DISTINCT worker.id, worker.name, birthday, sex, joining_date, position.name AS position, department.name AS department
--FROM worker
--JOIN worker_department
--ON worker.id = worker_department.worker_id
--LEFT JOIN department
--ON worker_department.department_id = department.id
--LEFT JOIN position
--ON worker.position_id = position.id
--ORDER BY department_id, position_id, joining_date ASC;
--
